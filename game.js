...
(function(){
  ...
  const GAME_LENGTH_MS = 2 * 60 * 1000; // 2 minutes
  let gameStart = 0;
  let gameEndAt = 0;
  ...
  window.addEventListener('keydown',(e)=>{ if(!e||!e.code) return; if(gameOver) return; const gameplay = ['KeyW','KeyA','KeyS','KeyD','ArrowUp','ArrowDown','ArrowLeft','ArrowRight','KeyF','KeyL']; if(gameplay.indexOf(e.code)!==-1) e.preventDefault(); keys[e.code]=true; lastKey=e.code; lastKeyAt=performance.now(); if(e.code === player1.controls.use && !player1.charging){ player1.charging = true; player1.chargeStart = performance.now(); } if(e.code === player2.controls.use && !player2.charging){ player2.charging = true; player2.chargeStart = performance.now(); } });
  window.addEventListener('keyup',(e)=>{ if(!e||!e.code) return; if(gameOver) return; keys[e.code]=false; if(e.code === player1.controls.use && player1.charging){ player1.charging = false; releaseUse(player1); } if(e.code === player2.controls.use && player2.charging){ player2.charging = false; releaseUse(player2); } });
  ...
  let last = performance.now(); function step(){ const now=performance.now(); const dt = now - last; last = now;
    if(!gameStarted){ gameStarted = true; gameStart = now; gameEndAt = gameStart + GAME_LENGTH_MS; }
    update(now, dt); draw(now); requestAnimationFrame(step); }
  function update(now, dt){ players.forEach(p=>p.update(keys, obstacles)); for(const atk of attacks){ atk.update(); for(const p of players){ if(p===atk.owner) continue; if(!p.alive) continue; if(rectsOverlap(atk.rect(), p.rect())){ p.takeDamage(atk.damage, atk.owner); /* hit visual suppressed */ atk.ttl = 0; break; } } } for(let i=attacks.length-1;i>=0;i--) if(attacks[i].ttl<=0) attacks.splice(i,1); /* hitEffects removed */ for(const p of players){ if(!p.alive && p.respawnAt === null){ p.respawnAt = now + 5000; if(p.item){ const typ = p.item.type; const uses_left = p.item.usesLeft || 0; const dropped = new Item(p.x + (p===player1 ? 20 : -20), p.y, typ); dropped.usesLeft = uses_left; items.push(dropped); p.item = null; } } } for(const p of players){ if(!p.alive && p.respawnAt !== null && now >= p.respawnAt){ p.alive = true; p.health = 10; const [rx,ry] = randomSpawnPosition(); p.x = rx; p.y = ry; p.respawnAt = null; } }
    // Timer and game-over check
    if(!gameOver){
      if(now >= gameEndAt){
        gameOver = true;
        // decide winner
        if(player1.kills > player2.kills) gameWinnerText = `${player1.name} wins! (${player1.kills} - ${player2.kills})`;
        else if(player2.kills > player1.kills) gameWinnerText = `${player2.name} wins! (${player2.kills} - ${player1.kills})`;
        else gameWinnerText = `Tie! (${player1.kills} - ${player2.kills})`;
  ...
  function draw(now){ if(bgImage && bgImage.complete && bgImage.naturalWidth){ try{ ctx.drawImage(bgImage,0,0,WIDTH,HEIGHT); }catch(e){ ctx.fillStyle=BG; ctx.fillRect(0,0,WIDTH,HEIGHT); } } else { ctx.fillStyle = BG; ctx.fillRect(0,0,WIDTH,HEIGHT); }
    // top-center large ticking countdown
    try{
  ...
  for(const a of attacks) a.draw && a.draw(ctx);
    // scoreboard (top-right) - glassy, larger, rounded and 20% translucent effect
    try{
      const sbW = 320, sbH = 110, sbMargin = 18;
      ...
      // shadow + base (gives the glass depth)
      ctx.save();
      ctx.shadowColor = 'rgba(0,0,0,0.25)';
      ctx.shadowBlur = 14;
      ctx.fillStyle = 'rgba(255,255,255,0.20)'; // ~20% translucent base
      roundRectPath(sbX, sbY, sbW, sbH, radius);
      ctx.fill();
      ctx.restore();
      ...
      // subtle inner darkening at bottom for depth
      const innerGrad = ctx.createLinearGradient(sbX, sbY + sbH * 0.4, sbX, sbY + sbH);
      innerGrad.addColorStop(0, 'rgba(0,0,0,0.03)');
      innerGrad.addColorStop(1, 'rgba(0,0,0,0.08)');
      ctx.save();
      roundRectPath(sbX + 1, sbY + 1, sbW - 2, sbH - 2, radius - 1);
      ...
      roundRectPath(sbX + 0.5, sbY + 0.5, sbW - 1, sbH - 1, radius);
      ctx.stroke();
      ctx.restore();

      // Title and player rows (larger fonts)
  ctx.fillStyle = '#022';
  ctx.font = 'bold 20px Arial';
  ctx.fillText('Scoreboard', sbX + 16, sbY + 30);
  // draw remaining time
  const nowT = performance.now();
  const remaining = Math.max(0, Math.floor((gameEndAt - nowT) / 1000));
  const mins = Math.floor(remaining/60); const secs = remaining % 60;
  ctx.font = 'bold 18px Arial';
  ctx.fillStyle = '#022';
  ctx.fillText(`${mins}:${secs.toString().padStart(2,'0')}`, sbX + sbW - 72, sbY + 30);
      ...
      ctx.fillStyle = '#000';
      // color box
      ctx.fillStyle = player1.color || '#000';
      ctx.fillRect(sbX + 16, rowY - 12, 18, 18);
      // name and kills
      ctx.fillStyle = '#022';
      ctx.fillText(`${player1.name}: ${player1.kills}`, sbX + 42, rowY + 2);

  // Player 2
      ctx.fillStyle = player2.color || '#000';
      ctx.fillRect(sbX + 180, rowY - 12, 18, 18);
      ctx.fillStyle = '#022';
      ctx.fillText(`${player2.name}: ${player2.kills}`, sbX + 206, rowY + 2);
